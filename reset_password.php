...
if($qbValidation->qbCheckEmail($_POST['email_id'])==0)
{
header('location: '.$base_url.'reset_password.php?err=001');
exit();
}
else
{
...
if(mysqli_num_rows($sql) > 0) {
	$rsql = mysqli_query($con, "INSERT INTO reset_password(member_id,code,date) VALUES('$member_id','$code','$time')");
$fname = $result['username'];

$subject = "Somebody requested a new password for your Quakbox account";
$message = "
<html>
<head>
...
<a href='".$base_url."new_password.php?u=".$member_id."&c=".$code."' style='text-decoration:none'>
...
<div>Somebody recently asked to reset your Quakbox password.</div>
<a href='".$base_url."new_password.php?u=".$member_id."&c=".$code."' style='color:#3b5998;text-decoration:none'>Click here to change your password.</a>
</td>
</tr>
</tbody>
</table>
...
<tbody>
<tr>
<td style='color:#3b5998;text-decoration:none'>
<a href='".$base_url."new_password.php?u=".$member_id."&c=".$code."' target='_blank' style='color:#3b5998;text-decoration:none'>
...
$mail = mail($email, $subject, $message, $headers) or die('error');
header('location: '.$base_url.'email-sent.php');exit();
}
else
{
header('location: '.$base_url.'reset_password.php?err=001');exit();
}
}
}
?>
...
<!--[if gt IE 8]><!--> <html class="no-js" lang=""> <!--<![endif]-->
    ...
    <div class="container">
	  ...
	  <div class="panel-heading">
	    <h3 class="panel-title"><?php echo $lang['Forgot Password'];?></h3>
	  </div>
	  <div class="panel-body">
	    <div class="row">
		  <form  method="post" name="form" id="form">        
        	<div class="col-lg-12"><h5><?php echo $lang['Forgot Password Message'];?></h5><br /></div>
		        
		        <div class="form-group">
		        <label for="email_id"><?php echo $lang['Email'];?></label>
		     	<input name="email_id" type="email" id="email_id" autocomplete="off" class="form-control" required="required"  placeholder="<?php echo  $lang['Email'];?>" size="40"  value="<?php if(isset($form)) { echo $form->value("login"); }?>">
		        </div>
			<div class="form-group">
		    	<div class=" col-sm-8">
