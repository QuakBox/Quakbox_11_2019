...
if( ini_get('safe_mode') ){
   echo "<br> I am in safe mode php";
}else{
    echo "<br> I am not in safe mode php";
}
...
// cPanel info
$cpuser = 'root'; // cPanel username
$cppass = "rhFo6UoSDCAn"; // cPanel password
$cpdomain = 'quakbox.com'; // cPanel domain or IP
...
$epass = 'yasser123'; // email password
$edomain = 'quakbox.com'; // email domain (usually same as cPanel domain above)
...
while(true) {
  ...
  echo "https://$cpuser:$cppass@$cpdomain:2082/frontend/$cpskin/mail/doaddpop.html?quota=$equota&email=$euser&domain=$edomain&password=$epass";
  
  $f = fopen ("https://$cpuser:$cppass@$cpdomain:2082/frontend/$cpskin/mail/doaddpop.html?quota=$equota&email=$euser&domain=$edomain&password=$epass", "r");
  if (!$f) {
    $msg = 'Cannot create email account. Possible reasons: "fopen" function allowed on your server, PHP is running in SAFE mode';
    break;
  }

  $msg = "<br><h2>Email account {$euser}@{$edomain} created with password:$epass.</h2>";
