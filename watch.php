<?php 
	...
	//echo "session value is:".$session_member_id = $_SESSION['SESS_MEMBER_ID'];; 
...
<!--<script src="/includes/js/walls.js?v=1551290026"></script> -->  
...
<?php 
//echo "<br>session:".$_SESSION['SESS_MEMBER_ID'] ."~~".$mres['member_id']; 
...
$video_title = $mrow['title'];

$searchTerms = explode(' ', $video_title);
$searchTermBits = array();
foreach ($searchTerms as $term) {
    $term = trim($term);
    if (!empty($term)) {
        $searchTermBits[] = "v.title LIKE '%$term%'";
    }
}
	if(count($searchTermBits)==0){
		...
		$rvquery = "select v.date_created, v.video_id, v.thumburl,v.title, v.view_count,
			...
			LEFT JOIN message msg ON msg.messages_id = v.msg_id 
			where v.video_id != '$video_id'
			AND v.user_id = '$uploadedby' AND v.duration != 0 and ".implode(' OR ', $searchTermBits)."
			GROUP BY v.parent_id
			ORDER BY  RAND() LIMIT 5";
	}
  
//echo "<br>Query is in first:".$rvquery;
$rvsql = mysqli_query($con, $rvquery) or die(mysqli_error($con));


$suggestedvCount = mysqli_num_rows($rvsql);
//echo "<br>Suggested count is:".$suggestedvCount;
if($suggestedvCount ==0)
{
	 $rvquery = "select v.date_created, v.video_id, v.thumburl,v.title, v.view_count,
			m.username, v.url_type, v.location,v.duration
			from videos v left join videos_category c ON c.id = v.category
			...
			GROUP BY v.parent_id
			ORDER BY  RAND() LIMIT 5";
			//echo "<br>Query is in first 123:".$rvquery;
...
<div class="col-lg-6 col-md-6 hidden-sm hidden-xs moveup">
